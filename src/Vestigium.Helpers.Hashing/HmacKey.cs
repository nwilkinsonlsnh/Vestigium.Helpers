...
/// <summary>
/// HMAC / KMAC key. Same object for HMAC-SHA-2, HMAC-SHA-3, and KMAC. Generate random bytes, take UTF-8 of a typed secret, or decode Base64.
/// Minimum 16 bytes. Default generate size is 32. Dispose zeros the key. A finalizer is last-chance ZeroMemory if Dispose is skipped.
/// Passing a Base64 string to <see cref="FromString"/> treats the letters as the key — use <see cref="FromBase64"/>.
/// <see cref="ToString"/> is length only. Export is explicit via <see cref="ToHexLower"/> / <see cref="ToBase64"/>.
/// </summary>
public sealed class HmacKey : IDisposable
{
    ...
    public override string ToString()
        => _disposed ? "HmacKey(disposed)" : $"HmacKey({_bytes!.Length} bytes)";
