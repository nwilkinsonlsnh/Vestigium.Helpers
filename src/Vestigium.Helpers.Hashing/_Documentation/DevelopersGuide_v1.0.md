...
[`Requirements_v1.0.md`](Requirements_v1.0.md) is the contract. SHA-256 default (lowercase hex), SHA-384/512 and SHA-3 opt-in, HMAC-SHA256 default with HMAC-SHA384 / HMAC-SHA512 opt-in, Argon2id PHC password verifiers, CRC-32 / CRC-64 / xxHash checksums, hex↔Base64 converters. Not encryption. HMAC is in this project; there is no `Vestigium.Helpers.Hmac` sibling. Checksums are `Checksum*`, never `Hash()`.
...
- Default HMAC is **HMAC-SHA256**. `HmacAlgorithm.Sha384` / `Sha512` are opt-in. Output 32 / 48 / 64 bytes.
...
**Status.** Implemented v1.2. Public surface is Identity, Probe, Hash/Verify string-bytes-file, HMAC-SHA256/384/512, HashPassword/VerifyPassword, Checksum*, HashingConvert, HmacKey.

## Files

| File | Role |
|---|---|
| `HashingHelper.cs` | Façade: Hash, HMAC, password, checksum, verify, Probe |
...
| `HmacKey.cs` / `HmacKeySize.cs` / `HmacAlgorithm.cs` | Generate / FromString / FromBase64 / FromBytes; SHA-256 / 384 / 512 |
...
HMAC is not AES. Lengths 16/32/64/128 are not “HMAC-128 vs HMAC-256.” 64 is the SHA-256 block; 128 is hashed down inside HMAC. HMAC-SHA384 / HMAC-SHA512 use the same key object.
