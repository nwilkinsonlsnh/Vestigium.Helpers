...
There is no `src/Vestigium.Helpers.Hashing.Demo` in the slnx. Do not `dotnet run` a Demo. Hosts initialize logging:
...
Contract: `dotnet test src/Vestigium.Helpers.Tests --filter FullyQualifiedName~Hashing`

See [`Requirements_v1.0_PR04.md`](Requirements_v1.0_PR04.md).

## Roadmap

| Version | Work |
|---|---|
| v1.0 | Engine + converters + Argon2id PHC |
...
Never: AES, SHA-256-as-password, inventing a hash, `ReadAllBytes` on a capture, a separate Hmac csproj, a Hashing.Demo project this wave.
