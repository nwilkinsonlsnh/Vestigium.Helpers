...
**Version:** 1.4  
**Status:** Accepted. No FileIo Demo project this wave.  
**Date:** 19 September 2026

[`Requirements_v1.0.md`](Requirements_v1.0.md) is the contract. [`ImplementationPlan_v1.2.md`](ImplementationPlan_v1.2.md) is the active packaging plan. Open `Vestigium.Helpers.slnx`. Implementation lives in `src/Vestigium.Helpers.FileIo/`.
...
The class library never calls `VestigiumLogger.Initialize`. The host does. Category `Helpers`, APPID `FileIo`. Job JSONL uses `correlationId = JobId` (`fio-` + 12 hex).

```csharp
VestigiumLogger.Initialize(cfg =>
{
    cfg.AppId = FileIoCatalog.AppId;          // or the host APPID; folder follows the host
    cfg.LogDirectory = logDir;
    cfg.MinimumDiskLevel = VestigiumLogLevel.Information;
    FileIoCatalog.Register(cfg);              // EVENTID block 12500–12610
    AnalyticsCatalog.Register(cfg);           // required when Stats / NumericSeries matter
});
```
...
Live `FileIoProgress` is chatty (UI). JSONL is sparse: job start (full recipe, optional `by` / `reason` properties), recon start/complete, consumers released, pause/resume/cancel, exception decisions only, a Progress line every 15 s, Stats at finalize, job end. Do not log a quiet Success per file.
...
At finalize FileIo constructs `NumericSeries` snapshots (sizes always; rates only when bytes actually moved). Empty bins are Count = 0 and null statistics, never NaN. Audit Mode records sizes, not rates. Charts may draw those series; FileIo does not reference Charts. Register `AnalyticsCatalog` on the same logger config when those lines should have catalog names.
...
FileIo takes `Vestigium.Helpers.Analytics` **1.0.1** as a NuGet package. `Vestigium.Logging` **1.7.1** arrives via repo-root `Directory.Build.props` (`$(VestigiumLoggingVersion)`); FileIo also lists that package so the csproj is readable without opening the props file.

There is **no** `Vestigium.Helpers.FileIo.Demo` in this wave. A host initializes Logging as above. Probe is `%TEMP%` only.
