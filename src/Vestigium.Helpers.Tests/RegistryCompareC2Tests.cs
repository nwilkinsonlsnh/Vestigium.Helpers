...
public sealed class RegistryCompareC2Tests : IDisposable
{
    ...
    public RegistryCompareC2Tests()
    {
        ...
        _right = @"Software\Vestigium\Helpers.Tests\" + Guid.NewGuid().ToString("N");
        ...
        RegistryHelper.Local.CreateKey(Hive, _right + @"\Child", confirm: true);
        RegistryHelper.Local.SetValue(Hive, _right, "Mark", "same", confirm: true);
    }

    public void Dispose()
    {
        RegistryHelper.Local.DeleteKey(Hive, _left, recursive: true, confirm: true);
        RegistryHelper.Local.DeleteKey(Hive, _right, recursive: true, confirm: true);
    ...
    public void Same_shape_is_related_and_has_no_delta_lines()
    {
        ...
        Assert.Equal(RegistryWriteStatus.Ok, RegistryHelper.WriteIndex(b, Hive, _right, confirm: true).Status);
    ...
    public void Different_roots_are_unrelated_and_stop()
    {
        ...
        Assert.Equal(RegistryWriteStatus.Ok, result.Status);
        var text = File.ReadAllText(outFile);
        Assert.Contains("Unrelated", text);
