...
/// <summary>
/// Services-only door into Vestigium.Logging. Passwords never appear. No-op until host Initialize.
/// </summary>
internal static class HelperLog
{
    public const string Category = ServicesCatalog.Category;
    ...
    public static string CurrentAppId => Scope.Value?.AppId ?? AppIds.Services;
    ...
    private static int EventId(VestigiumLogLevel level, string subcategory, string message)
    {
        if (message.Contains("probe complete", StringComparison.OrdinalIgnoreCase))
            return ServicesEvents.ProbeComplete;
        if (message.Contains("Enumerating service-control", StringComparison.OrdinalIgnoreCase)
            || string.Equals(subcategory, Subcategories.Probe, StringComparison.Ordinal))
            return ServicesEvents.ProbeEnter;

        return level switch
        {
            VestigiumLogLevel.Debug or VestigiumLogLevel.Verbose => ServicesEvents.OperationEnter,
            VestigiumLogLevel.Warning => ServicesEvents.OperationWarning,
            VestigiumLogLevel.Error or VestigiumLogLevel.Fatal => ServicesEvents.OperationFailed,
            _ => ServicesEvents.OperationComplete
        };
    }

    private static string CatalogMessage(int eventId) => eventId switch
    {
        ServicesEvents.ProbeEnter => "enter Probe",
        ServicesEvents.ProbeComplete => "probe complete",
        ServicesEvents.OperationEnter => "enter operation",
        ServicesEvents.OperationComplete => "operation complete",
        ServicesEvents.OperationFailed => "operation failed",
        ServicesEvents.OperationWarning => "operation warning",
