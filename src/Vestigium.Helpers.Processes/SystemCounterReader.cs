...
internal static class SystemCounterReader
{
    ...
    internal static SystemCounters Capture(SystemRawSnapshot? previous, TimeSpan? interval, out SystemRawSnapshot raw)
    {
        ...
        long? ioPerSec = null;
        if (interval is { TotalSeconds: > 0 } span && readBytesDelta is { } r && writeBytesDelta is { } w && otherBytesDelta is { } o)
            ioPerSec = (long)((r + w + o) / span.TotalSeconds);

        double? commitPct = commitCurrent is { } cur && commitLimit is { } lim and > 0 ? (double)cur / lim : null;
        double? physPct = physTotal is { } tot && physAvail is { } av && tot > 0 ? (double)(tot - av) / tot : null;
        ...
        return new SystemCounters
        {
            ...
            CommitPeakToLimit = commitPeak is { } peak && commitLimit is { } lim2 and > 0 ? (double)peak / lim2 : null,
            PhysicalTotal = physTotal,
            PhysicalAvailable = physAvail,
            CacheWorkingSet = cacheWs,
            KernelWorkingSet = kernelWs,
            DriverWorkingSet = kernelWs is { } k && cacheWs is { } c ? Math.Max(0, k - c) : null,
