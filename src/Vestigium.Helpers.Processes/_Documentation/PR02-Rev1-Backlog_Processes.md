...
Phase C: `ProcessDeltaMap` fills `CPU.Usage` / `CPU.TimeDelta` / MEM and IO deltas on the second sample. Query watcher and campaign ticks use it. Closed windows clear the map. Tick exceptions log `Watch query tick failed type=…` (no query body) then raise empty `Matches`.
