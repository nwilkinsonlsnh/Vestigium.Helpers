...
internal static class ClosedXmlLog
{
        ...
        IReadOnlyDictionary<string, string?>? properties = null, string? appId = null)
    {
        if (!VestigiumLogger.IsInitialized)
            return;
        VestigiumLog.Debug(eventId, VestigiumStatus.Pending, ClosedXmlCatalog.Category, subcategory, message,
            correlationId, properties, appId ?? ClosedXmlCatalog.AppId);
    }
        ...
        IReadOnlyDictionary<string, string?>? properties = null, string? appId = null)
    {
        if (!VestigiumLogger.IsInitialized)
            return;
        VestigiumLog.Information(eventId, VestigiumStatus.Success, ClosedXmlCatalog.Category, subcategory, message,
            correlationId, properties, appId ?? ClosedXmlCatalog.AppId);
    }
        ...
        IReadOnlyDictionary<string, string?>? properties = null, string? appId = null)
    {
        if (!VestigiumLogger.IsInitialized)
            return;
        VestigiumLog.Warning(eventId, VestigiumStatus.Warning, ClosedXmlCatalog.Category, subcategory, message,
            correlationId: correlationId, properties: properties, appId: appId ?? ClosedXmlCatalog.AppId);
    }
        ...
        string? correlationId = null, IReadOnlyDictionary<string, string?>? properties = null, string? appId = null)
    {
        if (!VestigiumLogger.IsInitialized)
            return;
        VestigiumLog.Error(eventId, VestigiumStatus.Failed, ClosedXmlCatalog.Category, subcategory, message,
            exception, correlationId, properties, appId ?? ClosedXmlCatalog.AppId);
    }
    ...
    public static void Unexpected(int eventId, string subcategory, Exception exception, string? correlationId = null, string? appId = null)
    {
        if (exception is ArgumentException or ArgumentOutOfRangeException or ArgumentNullException
            or InvalidOperationException or FileNotFoundException or KeyNotFoundException or ObjectDisposedException)
            return;
        Error(eventId, subcategory, "unexpected failure", exception, correlationId, appId: appId);
    }

    public static string RequireNotBlank(string? value, string paramName, int eventId, string subcategory)
    {
        if (!string.IsNullOrWhiteSpace(value))
            return value.Trim();
        Error(eventId, subcategory, "rejected session", properties: Props(("param", paramName), ("reason", "blank")));
        throw new ArgumentException("Value is required.", paramName);
    }

    public static string RequireFile(string? path, string paramName)
    {
        var target = RequireNotBlank(path, paramName, ClosedXmlEvents.SessionRejected, ClosedXmlCatalog.Subcategories.Session);
        if (File.Exists(target))
            return target;
        Error(ClosedXmlEvents.SessionRejected, ClosedXmlCatalog.Subcategories.Session, "rejected session",
