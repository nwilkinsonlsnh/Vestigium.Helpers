...
2. Tabs: Overview, **AES-256-GCM**, **ChaCha20-Poly1305**, **AES-256-CBC + HMAC**, **Argon2id**, **Key ring**, **Round trip**, Validate, JSONL.
...
6. **Key ring** tab: generate an operator pair; Issue CompanyX / ApplicationX and CompanyX / ApplicationY (2048-bit in the gallery with a caption that the library default is 3072); optional escrow; Enable / Disable / Expire / Retire / Compromise; export public slip and ring JSON (no PKCS8). Issue is issue-and-forget on the operator ring (public contact kept; private slip held in the gallery). **Round trip** tab: Seal to one or more contacts; optional also wrap to Ops; Open as Ops / AppX / AppY / stranger to show isolation; Peek wrap thumbprints. **Token edition:** Disabled or expired Seal/Open via the ring requires an override (requestedBy + reason); a warning is logged. Compromised / retired cannot be overridden. JSONL never shows issuedTo or PKCS8.
