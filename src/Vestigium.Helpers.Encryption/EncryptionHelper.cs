...
public static class EncryptionHelper
{
    ...
    {
        var source = EncryptionLog.RequireNotBlank(sourcePath, nameof(sourcePath));
        var destArg = EncryptionLog.RequireNotBlank(destinationPath, nameof(destinationPath));
        ArgumentNullException.ThrowIfNull(secret);
    ...
    public static void SecureDelete(string path, SecureDeleteMode mode)
    {
        ...
        var file = EncryptionLog.RequireNotBlank(path, nameof(path));
    ...
    private static FileStream OpenRead(string path)
        => new(EncryptionLog.RequireNotBlank(path, nameof(path)), FileMode.Open, FileAccess.Read, FileShare.Read);
